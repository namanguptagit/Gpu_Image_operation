# CubeCL Image Operation Benchmark

This project demonstrates a single GPU-accelerated image operation (RGB to Grayscale conversion) in Rust using [CubeCL](https://github.com/tracel-ai/cubecl), benchmarked against the CPU implementation from [kornia-rs](https://github.com/kornia/kornia-rs).

It fulfills the objective of creating a simple, done-properly GPU operation in Rust using CubeCL and measuring it against a reliable baseline.

## Implementation Details

- **CPU Baseline:** Uses `imgproc::color::gray_from_rgb` from `kornia-imgproc`.
- **GPU Kernel:** Uses `#[cube(launch_unchecked)]` to compile a WGPU-compatible shader directly from Rust.
- **Validation:** Ensures that both CPU and GPU backends generate the exact identical output tensors up to precision limits.
