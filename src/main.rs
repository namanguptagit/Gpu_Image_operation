...
pub fn main() {
    ...
    let cpu_result =
        cpu::run_cpu_benchmark(width as usize, height as usize, &image_data, warmup, iters);
    let gpu_result =
        gpu::run_gpu_benchmark(width as usize, height as usize, &image_data, warmup, iters);
    ...
    let gpu_raw: Vec<u8> = gpu_result
        .iter()
        .map(|&p| (p.clamp(0.0, 1.0) * 255.0) as u8)
        .collect();
    let gpu_img = ImageBuffer::<Luma<u8>, Vec<u8>>::from_raw(width, height, gpu_raw)
